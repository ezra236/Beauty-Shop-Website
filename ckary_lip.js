...
function updateMonicaContent(headerText, paragraphText, buttonLink) {
  ...
  function toggleBox(clickedBox) {
    ...
    if (clickedBox.classList.contains('clicked')) {
      ...
      if (clickedBox === box1) {
        monica.style.left = '5%';
        updateMonicaContent('YOUR WINNING ROUTINE', 'Whether you got dry, oil or combo skin', 'lip1.html');
      } else if (clickedBox === box2) {
        monica.style.left = '21%';
        updateMonicaContent('ALWAYS BETTER TOGETHER', 'Its legendary lipcare', 'lip2.html');
      } else if (clickedBox === box3) {
        monica.style.left = '38%';
        updateMonicaContent('HAIR FOR THE HYDRATION', 'Secure Our reparative lineup', 'lip3.html');
      } else if (clickedBox === box4) {
        monica.style.left = '55%';
        updateMonicaContent('BOLD SCENT YOU WONT FORGET', 'Step out in Ckay spicy', 'lip4.html');
      }
    }
