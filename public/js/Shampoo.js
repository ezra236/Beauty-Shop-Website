...
document.addEventListener('DOMContentLoaded', function () {
    const imageSources = {
        ...
        item2: { original: '/images/drop.jpg', hover: '/images/b11.jpg' },
        item3: { original: '/images/lip.jpg', hover: '/images/lip2.jpg' },
        item4: { original: '/images/bm3.jpg', hover: '/images/ap2.jpg' },
        item5: { original: '/images/g.jpg', hover: '/images/ap3.jpg' },
        item6: { original: '/images/b20.jpg', hover: '/images/ap1.jpg' },
        item7: { original: '/images/bm1.jpg', hover: '/images/lips.jpg' },
        item8: { original: '/images/skinca.jpg', hover: '/images/ap3.jpg' }
    };

    // Function to change the image source
    function changeImage(itemId, newSrc) {
        const itemImage = document.getElementById(itemId);
        if (!itemImage) return; // Exit if element not found

        // Add fade-out effect
        itemImage.classList.add('fade-out');

        // Change image after transition
        setTimeout(() => {
            itemImage.src = newSrc;
            itemImage.classList.remove('fade-out'); // Remove fade-out effect
        }, 100); // Matches the transition duration
    }

    // Select all images and attach event listeners
    document.querySelectorAll('.classic-item img').forEach(itemImage => {
        const itemId = itemImage.id;
        if (imageSources[itemId]) { // Ensure the item ID exists in the object
            itemImage.addEventListener('mouseenter', () => changeImage(itemId, imageSources[itemId].hover));
            itemImage.addEventListener('mouseleave', () => changeImage(itemId, imageSources[itemId].original));
        }
    });
});
