...
$sql = "SELECT image_path FROM promo_content4 WHERE id = 1"; // Fetching the first record as an example
...
<head>
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('css/productD.css') }}">
    </head>
...
<div class="inform">
    ...
    <nav class="breadcrumb">
        <a href="{{ route('Home') }}">Home</a> / <a href="{{ url('/productE') }}">Cherry Dub Pore Purify'r Gel Cleanser with Niacinamide + Aloe Juice</a>
    </nav>
...
$sql = "SELECT review, headline, name, age_range, gender, created_at FROM reviews3 ORDER BY id DESC LIMIT 1";
$result = $conn->query($sql);
$row = $result->fetch_assoc();
?>
...
<script src="{{ asset('js/productD.js') }}"></script>
